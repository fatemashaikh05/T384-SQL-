...
ALTER TABLE Branches ADD CONSTRAINT PK_Branches
PRIMARY KEY (BranchID);

ALTER TABLE Loans ADD CONSTRAINT PK_Loans
PRIMARY KEY (LoanID);
...
ALTER TABLE Loans ADD CONSTRAINT FK_Loans_Customers
FOREIGN KEY (CustomerID) REFERENCES Customers(CustomerID);

ALTER TABLE Accounts ADD BranchID INT;

ALTER TABLE Accounts ADD CONSTRAINT FK_Accounts_Branches
FOREIGN KEY (BranchID) REFERENCES Branches(BranchID);

INSERT INTO Customers (CustomerID, FirstName, LastName, Email, Phone, DateOfBirth)
VALUES (101,'Rahul','Sharma','rahul@gmail.com','9876543210','1998-04-15');
...
-- Sub Queries --
-- Q.Find the employees having same salary as that of James Brown --
use bankingdb;
...
SELECT SUBSTRING(FullName, 2, 1) FROM Employee where employeeid=1010;
SELECT * FROM Employee WHERE substring(FullName,2,1) = (SELECT SUBSTRING(FullName, 2, 1) FROM Employee where employeeid=1010);
...
select * from employee where salary > (select salary from employee where fullname="Anurag Mohanty");

select * from employee where department in ("IT","HR");

-- >ANY-'grater than minimum' & <ANY-'less than maximum' --
select * from employee where age<26 or age<28; -- ANY OR ke hisaab se kaam karta hai --
-- same output as above query --
select * from employee where age < any(select age from employee where fullname="James Brown" or fullname="Linda Jones");

select * from employee where age<26 and age<28; -- ALL AND ke hisaab se kaam karta hai --
-- same output as above query --
...
select *, datediff(EndDate,StartDate) from projects where Duration>10 or Duration>15;

SELECT *, datediff(EndDate,StartDate) AS Duration FROM Projects
WHERE ProjectId IN ( SELECT ProjectId FROM Projects WHERE datediff(EndDate,StartDate) > 10 or datediff(EndDate,StartDate) > 15);
